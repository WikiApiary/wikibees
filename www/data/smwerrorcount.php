...
try {
    ...
    $stmt = $conn->prepare('SELECT capture_date, errorcount FROM smwinfo WHERE website_id = :id AND capture_date > :date_filter AND errorcount IS NOT NULL ORDER BY capture_date ASC');
    $stmt->execute(array('id' => $id, 'date_filter' => $date_filter));
    $result = $stmt->fetchAll();
    if ( count($result) ) {
        foreach($result as $row) {
            printf ("%s, %s\n",
                $row['capture_date'], $row['errorcount']);
        }
    } else {
